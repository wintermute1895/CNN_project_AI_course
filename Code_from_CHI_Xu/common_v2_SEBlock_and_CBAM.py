        ...
        # PaddlePaddle
            ...
            if not (model_file and params_file and model_file.is_file() and params_file.is_file()):
                raise FileNotFoundError(
                    f"Model files not found in {w}. Both .pdmodel and .pdiparams files are required.")
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            ...
            if self.dynamic and im.shape != self.bindings["images"].shape:
                i = self.model.get_binding_index("images") if hasattr(self.model,
                                                                      'get_binding_index') else self.model.get_tensor_name(
                    0)  # TRT 10 compatibility
                self.context.set_binding_shape(i, im.shape) if hasattr(self.context,
                                                                       'set_binding_shape') else self.context.set_input_shape(
                    i, im.shape)  # TRT 10 compatibility
                self.bindings["images"] = self.bindings["images"]._replace(shape=im.shape)
                for name_idx, name in enumerate(self.output_names):  # Iterate over sorted output names
                    i_out = self.model.get_binding_index(name) if hasattr(self.model,
                                                                          'get_binding_index') else self.model.get_tensor_name(
                        len(self.bindings) - len(self.output_names) + name_idx)  # TRT 10 compatibility
                    self.bindings[name].data.resize_(tuple(
                        self.context.get_binding_shape(i_out) if hasattr(self.context,
                                                                         'get_binding_shape') else self.context.get_tensor_shape(
                            i_out)))  # TRT 10 compatibility
...
class AutoShape(nn.Module):
    ...
    def __init__(self, model, verbose=True):
        ...
        if self.pt:  # Check if pt model before accessing model.model
            ...
            if hasattr(model, 'model') and model.model is not None:
                ...
                LOGGER.warning(
                    "AutoShape: Model structure not as expected for PyTorch model, Detect() module attributes may not be set.")
    ...
    def forward(self, ims, size=640, augment=False, profile=False):
        ...
        """
        dt = (Profile(), Profile(), Profile()) if profile else [
                                                                   contextlib.nullcontext()] * 3  # Use Profile only if profile=True
        with dt[0]:  # Preprocessing
            ...
            for i, im_item in enumerate(ims_list):
                ...
                if isinstance(im_item, (str, Path)):  # filename or uri
                    im_pil, f = Image.open(
                        requests.get(im_item, stream=True).raw if str(im_item).startswith("http") else im_item), im_item
                ...
                im_np = im_np[..., :3] if im_np.ndim == 3 and im_np.shape[-1] >= 3 else \
                    cv2.cvtColor(im_np, cv2.COLOR_GRAY2BGR) if im_np.ndim == 2 else \
                        (np.zeros((size[0], size[1], 3),
                                  dtype=np.uint8) if im_np.size == 0 else im_np)  # Handle grayscale and empty, enforce 3ch
            ...
            x_stacked = np.ascontiguousarray(np.array(x_padded).transpose((0, 3, 1, 2))) if x_padded else np.array(
            ...
            if x_stacked.size == 0:  # Handle case with no images after preprocessing
                ...
                return Detections(ims_list, [torch.empty(0, 6, device=p_device)], files, dt, self.names,
                                  (n, 3, inf_shape[0], inf_shape[1]) if n > 0 else (0, 3, inf_shape[0], inf_shape[1]))
...
class Detections:
    ...
    def __init__(self, ims, pred, files, times=(0.0, 0.0, 0.0), names=None, shape=None):  # Ensure times is float tuple
        ...
        for i, im_item in enumerate(ims):
            if hasattr(im_item, 'shape') and i < len(pred_list) and isinstance(pred_list[i], torch.Tensor):
                ...
                gn.append(torch.tensor([im_shape[1], im_shape[0], im_shape[1], im_shape[0], 1.0, 1.0], device=d,
                                       dtype=torch.float32))
        ...
        self.t_formatted = (self.t_tuple[0] / self.n if self.n > 0 else 0.0,
                            ...
                            self.t_tuple[2] / self.n if self.n > 0 else 0.0) if len(self.t_tuple) == 3 else (
            0.0, 0.0, 0.0)
        ...
        self.conf = [p[:, 4] if p.ndim > 1 and p.shape[0] > 0 and p.shape[1] > 4 else torch.empty(0, device=d) for p in
                     self.pred]
        self.cls = [p[:, 5] if p.ndim > 1 and p.shape[0] > 0 and p.shape[1] > 5 else torch.empty(0, device=d) for p in
                    self.pred]

        # Normalized boxes (handle cases where gn might be empty if self.pred was empty)
        self.xyxyn = [(x / g[:4] if g.numel() > 0 else torch.empty_like(x)) for x, g in zip(self.xyxy, gn)] if gn else [
            torch.empty_like(x) for x in self.xyxy]

        # WH formats (handle empty predictions)
        self.xywh = [xyxy2xywh(x) if x.numel() > 0 else torch.empty_like(x) for x in self.xyxy]
        self.xywhn = [(x / g[:4] if g.numel() > 0 else torch.empty_like(x)) for x, g in zip(self.xywh, gn)] if gn else [
            torch.empty_like(x) for x in self.xywh]
    ...
    def _run(self, pprint=False, show=False, save=False, crop=False, render=False, labels=True, save_dir=Path(".")):
        ...
        for i, (im_orig, pred_tensor) in enumerate(zip(self.ims, self.pred)):
            ...
            if pred_tensor.shape[0]:  # If there are detections
                ...
                unique_classes = pred_tensor[:, -1].unique() if pred_tensor.ndim > 1 and pred_tensor.shape[
                    1] > 5 else torch.empty(0)

                for c_val in unique_classes:
                    n_det = (pred_tensor[:, -1] == c_val).sum()  # detections per class
                    class_name_str = self.names.get(int(c_val.item()),
                                                    f"class_{int(c_val.item())}")  # Use .get for safety
                ...
                if show or save or render or crop:
                    ...
                    for *box_coords, conf_val, cls_val in reversed(
                            pred_tensor.tolist()):  # Convert to list for easier iteration
                            ...
                            crops_list.append(
                                {
                                    ...
                                    "im": save_one_box(torch.tensor(box_coords, device=pred_tensor.device),
                                                       im_orig.copy(), file=crop_save_path, BGR=True, save=save),
                                }
                            )
                        else:  # all others (show, save, render)
                            annotator.box_label(box_coords, label_text if labels else "",
                                                color=colors(int(cls_val), True))
            ...
            im_pil = Image.fromarray(cv2.cvtColor(im_result, cv2.COLOR_BGR2RGB)) if isinstance(im_result,
                                                                                               np.ndarray) else im_result
    ...
    def __repr__(self):
        ...
        class_name_repr = self.__class__.__name__ if hasattr(self, '__class__') and hasattr(self.__class__,
                                                                                            '__name__') else 'Detections'
...
# ... (ChannelAttention 和 SpatialAttention 的定义可以保持不变，因为它们被CBAM内部调用) ...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):  # in_planes 是实际的输入通道数
        super(ChannelAttention, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        self.shared_mlp = nn.Sequential(
            nn.Conv2d(in_planes, max(1, in_planes // ratio), 1, bias=False),  # 确保中间通道至少为1
            nn.SiLU(),  # 建议使用SiLU与YOLOv5风格统一
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_map = self.conv(x_cat)
        return self.sigmoid(spatial_map)


# --- 修改 SEBlock ---
class SEBlock(nn.Module):
    # channels_from_yaml: 这个参数将接收来自 YAML 文件中 args[0] 的值。
    #                     我们期望这个值是该SEBlock模块的实际输入通道数。
    # ratio_from_yaml: 这个参数将接收来自 YAML 文件中 args[1] 的值 (如果提供的话)。
    def __init__(self, channels_from_yaml, ratio_from_yaml=16):
        super(SEBlock, self).__init__()

        actual_input_channels = channels_from_yaml  # SEBlock的输入和输出通道数相同
        reduction_ratio = ratio_from_yaml

        # LOGGER.info(f"SEBlock initialized: actual_input_channels={actual_input_channels}, reduction_ratio={reduction_ratio}")

        self.squeeze = nn.AdaptiveAvgPool2d(1)
        self.excitation = nn.Sequential(
            nn.Conv2d(actual_input_channels, max(1, actual_input_channels // reduction_ratio), 1, bias=False),
            nn.SiLU(),
            nn.Conv2d(max(1, actual_input_channels // reduction_ratio), actual_input_channels, 1, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x):  # x 的输入通道数将是初始化时的 actual_input_channels
        return x * self.excitation(self.squeeze(x))


# --- 修改 CBAM ---
class CBAM(nn.Module):
    # channels_from_yaml: 接收 YAML args[0]，期望是此CBAM的实际输入通道数。
    # ratio_from_yaml: 接收 YAML args[1] (如果提供)。
    # kernel_size_from_yaml: 接收 YAML args[2] (如果提供)。
    def __init__(self, channels_from_yaml, ratio_from_yaml=16, kernel_size_from_yaml=7):
        super(CBAM, self).__init__()

        actual_input_channels = channels_from_yaml  # CBAM的输入和输出通道数相同
        ratio = ratio_from_yaml
        kernel_size = kernel_size_from_yaml

        # LOGGER.info(f"CBAM initialized: actual_input_channels={actual_input_channels}, ratio={ratio}, kernel_size={kernel_size}")

        self.channel_attention = ChannelAttention(actual_input_channels, ratio)
...
# ... (确保所有原始 common.py 中的其他模块定义也在这里，或者你能确保它们能从原始 common.py 加载)